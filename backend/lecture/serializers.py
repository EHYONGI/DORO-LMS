...
class LectureSerializer(serializers.ModelSerializer):
    ...
    # 커리큘럼 관련 필드
    competency_type_display = serializers.CharField(
        source='get_competency_type_display', read_only=True
    )
    ...
    def get_instructor_name(self, obj):
        """
        강사 이름: instructor.last_name + instructor.first_name
        둘 다 비어 있으면 username 사용
        """
        if not obj.instructor:
            return ''
        last_name = (obj.instructor.last_name or '').strip()
        first_name = (obj.instructor.first_name or '').strip()
...
class LectureNoticeSerializer(serializers.ModelSerializer):
    ...
    lecture_name = serializers.ReadOnlyField(source='lecture.name')
    # 강사 이름도 last_name + first_name으로 맞춤
    ...
    def get_author_name(self, obj):
        """
        공지 작성자 이름: lecture.instructor.last_name + lecture.instructor.first_name
        둘 다 비어 있으면 username 사용
        """
        instructor = getattr(obj.lecture, 'instructor', None)
        if not instructor:
            return ''
        last_name = (instructor.last_name or '').strip()
        first_name = (instructor.first_name or '').strip()
        full_name = f"{last_name}{first_name}".strip()
